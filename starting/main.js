    ...
    question() {
      ...
      playRole() {
        ...
        //if input is out of the map
          ...
          // } else if (this.field[this.y][this.x] === this.field[0][0]) {
          //   Math.floor(Math.random * this.x);
          } else {
            //put the "*" in the [this.y] [this.x]
       ...
       static generateField(height, width, percentage) {
         ...
         while (bigArray.length < height) {
           //  for (let i = 0; i < height; i += 1) {
            ...
            // if (startPoint(Math.floor(Math.random() * width)));
            // bigArray[0][0] == startPoint;
            
            // bigArray[0][Math.floor(Math.random() * width)] = pathCharacter;
